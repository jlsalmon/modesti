...
public class Constraints {
  ...
  private static boolean validateOrConstraint(Constraint constraint, Request request, Category category) {
    ...
    for (Point point : request.getPoints()) {
      ...
      List<Field> fields = SchemaUtils.getFields(category, constraint.getMembers());

      // Get a list of fields for the constraint that are empty
      List<Field> emptyFields = SchemaUtils.getEmptyFields(point, fields);
      ...
      if (emptyFields.size() == constraint.getMembers().size()) {
        ...
        for (Field emptyField : emptyFields) {
          point.addErrorMessage(SchemaUtils.getPropertyName(emptyField), "At least one of '" + String.join(", ", fieldNames)
  ...
  private static boolean validateAndConstraint(Constraint constraint, Request request, Category category) {
    ...
    for (Point point : request.getPoints()) {
      ...
      if (emptyFields.size() > 0) {
        ...
        for (Field emptyField : emptyFields) {
          point.addErrorMessage(SchemaUtils.getPropertyName(emptyField),
    ...
    for (Point point : request.getPoints()) {
      ...
      if (value != null && !value.isEmpty() && concatenatedValues.indexOf(value) != concatenatedValues.lastIndexOf(value)) {
        ...
        if (constraint.getMembers().size() == 1) {
          point.addErrorMessage(constraint.getMembers().get(0), "Field '" + fieldNames.get(0) + "' must be unique for all points. Check for duplications.");
        } else {
          point.addErrorMessage("", "Field(s) '" + String.join(", ", fieldNames) + "' must be unique for all points. Check for duplications.");
        }
      }
    }
