 ...
 *  - Test WINCC workflow
 */
public class CoreWorkflowServiceTest extends BaseIntegrationTest {
  ...
  public void createPoints() throws Exception {
    ...
    ProcessInstance process = coreWorkflowService.startProcessInstance(request);
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "edit", "IN_PROGRESS");
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "submit", "IN_PROGRESS");
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "configure", "FOR_CONFIGURATION");
...
//    }).when(configurationService).configureRequest(anyString(), anyObject());
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "test", "FOR_TESTING");
...
//    request.setTesting(new Testing(true, ""));
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), null, "CLOSED");
  ...
  public void createAlarms() throws Exception {
    ...
    completeCurrentTask(request.getRequestId(), "submit", BEN);
    assertTaskNameAndRequestStatus(request.getRequestId(), "edit", "FOR_APPROVAL");
...
//    request.setApproval(new Approval(true, ""));
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "submit", "FOR_APPROVAL");
...
//    request.setTesting(new Testing(true, ""));
  ...
  public void createCabledPoints() throws Exception {
    ...
    completeCurrentTask(request.getRequestId(), "submit", BEN);
    assertTaskNameAndRequestStatus(request.getRequestId(), "edit", "FOR_ADDRESSING");
...
//    request.setAddressing(new Addressing(true, ""));
    ...
    assertTaskNameAndRequestStatus(request.getRequestId(), "submit", "FOR_ADDRESSING");

    // Completing the 'submit' task should lead to the 'cable' task
    completeCurrentTask(request.getRequestId(), "submit", ROB);
    assertTaskNameAndRequestStatus(request.getRequestId(), "cable", "FOR_CABLING");
