...
import cern.modesti.schema.spi.SchemaPostProcessor;
...
public class SchemaInitialiser {
   ...
   * @throws URISyntaxException
   */
  private Map<String, List<SchemaImpl>> loadPluginSchemas() throws IOException, URISyntaxException {
    log.info("Loading plugin schemas");
    return loadPluginResources(PLUGIN_RESOURCE_PREFIX + SCHEMA_RESOURCE_PATTERN, SchemaImpl.class);
  }
   ...
   * @throws URISyntaxException
   */
  private Map<String, List<CategoryImpl>> loadPluginCategories() throws IOException, URISyntaxException {
    log.info("Loading plugin categories");
    return loadPluginResources(PLUGIN_RESOURCE_PREFIX + CATEGORY_RESOURCE_PATTERN, CategoryImpl.class);
  }
   ...
   * @throws URISyntaxException
   */
  private Map<String, List<DatasourceImpl>> loadPluginDatasources() throws IOException, URISyntaxException {
    log.info("Loading plugin datasources");
    return loadPluginResources(PLUGIN_RESOURCE_PREFIX + DATASOURCE_RESOURCE_PATTERN, DatasourceImpl.class);
  }
  ...
  private Schema getSchema(Map<String, List<SchemaImpl>> schemas, String schemaId) {
    for (Map.Entry<String, List<SchemaImpl>> entry : schemas.entrySet()) {
